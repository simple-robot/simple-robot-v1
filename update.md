...
# 1.16.0
- quartz 版本更新到2.3.2
- 预定将会集成cqcodeutils模组并弃用原版的CQCodeUtil
- 对`@Filter(at = true)`时候的情况做一些特殊处理
- 修复`@Filter`动态参数提取的bug
- 追加注解`@Async(...)`(异步函数)，标注在监听函数上以表示此函数为异步执行。异步执行的线程又初始化的线程池控制。
- 追加注解`@Limit(...)`(限流函数)，标注监听函数在多长一段时间内可触发。例如`@Limit(5)`，则代表5秒内只会触发一次。此功能基于监听函数拦截器实现。
