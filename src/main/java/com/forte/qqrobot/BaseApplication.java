...
public abstract class BaseApplication<CONFIG extends BaseConfiguration> {
     ...
     * 线程工厂初始化
     */
    protected final void threadPoolInit(){
     ...
     * 公共资源初始化
     */
    protected final void baseResourceInit(){
        //将CQCodeUtil放入资源调度中心
     ...
     * 配置结束后的方法
     */
    private void afterConfig(CONFIG config){
        //构建监听函数管理器等扫描器所构建的
    ...
    public void run(Application<CONFIG> app){
        ...
        //配置结束
        afterConfig(configuration);

        //连接之后，如果没有进行扫描，则默认扫描启动类同级包且排除此启动类
        String packageName = app.getClass().getPackage().getName();

        //如果没有扫描过，扫描本包全部
        configuration.scannerIfNotScanned(packageName, c -> !c.equals(app.getClass()));
