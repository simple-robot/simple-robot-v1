 ...
 * File     SenderInterceptFactory.java
 *
...
//    private static Map<SenderSetList, SenderSetList>  SETTER_CACHE = new HashMap<>(2);
    ...
    public static SenderSendList doSenderIntercept(SenderSendList sender, SenderSendIntercept... intercepts) {
        return doIntercept(SenderSendList.class, sender, (v, m, p) -> new SendContext(v, m, p), intercepts);
    }
    ...
    public static SenderGetList doGetterIntercept(SenderGetList sender, SenderGetIntercept... intercepts) {
        return doIntercept(SenderGetList.class, sender, GetContext::new, intercepts);
    }
    ...
    public static SenderSetList doSetterIntercept(SenderSetList sender, SenderSetIntercept... intercepts) {
        return doIntercept(SenderSetList.class, sender, SetContext::new, intercepts);
    }
     ...
     * @return 代理对象
     */
    public static <S extends SenderList> S doIntercept(Class<? extends S> type, S sender, ExFunction<S, Method, Object[], SenderContext<? extends S>> contextGetter, Interceptor[] intercepts) {
        // 先对数组进行排序
        if (intercepts.length > 0) {
            ...
            return senderProxy(type, sender, (pro, m, args) -> {
                ...
                // 代理
                SenderContext<?> context = contextGetter.apply(sender, m, args);
                // 执行前置
